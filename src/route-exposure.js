/*
 * Haidian Soundscape — Route Exposure Foundation v9.0.0-dev37.4 Pedestrian Realm Graph Rescue
 *
 * Capabilities:
...
(function () {
  ...
  const DEFAULTS = {
    ...
    // dev36.1: extreme stretch is first audited with cheap topology-only
    // fastest probes. If strict public-foot routing still detours, run two
    // bounded rescue layers: (1) live OSM access=private-as-conditional, never
    // when foot=no/private; (2) independent Valhalla pedestrian cross-check.
    routeStretchRescue: {
      enabled: true,
      ratioThreshold: 2.4,
      minExcessM: 250,
      ...
      // Very short A→B with a ≥4× first-stage detour is almost always a local
      // access/topology semantics problem, not a reason to fetch 25 HGR2 tiles.
      // Jump to focused live OSM / conditional-access cross-check first.
      ...
      // dev36.3: if no dangling endpoint defect exists, inspect exact same-grade
      // interior crossings in strict live OSM, then test a detached OSM×Overture
      // corridor union with only tiny independent-source near-touch stitches.
  ...
  let lastCandidateCorrectnessAudit = null;
  let drawMode = "idle";
  let drawPoints = [];
  let aPoint = null;
  let bPoint = null;
  ...
  function routeShadeWarmCacheContext() {
    ...
    return {
      ...
      modelReady: cacheable,
      modelToken: String(model?.token || ''),
      modelContext: model,
      shadeTimeBucketSec: graphCfg.shadeTimeBucketSec,
      shadeSampleSpacingM: graphCfg.shadeSampleSpacingM,
      shadeMaxSamplesPerEdge: graphCfg.shadeMaxSamplesPerEdge,
      ttlMs: graphCfg.sessionShadeWarmCacheTtlMs,
      maxEntries: graphCfg.sessionShadeWarmCacheMaxEntries,
      maxNamespaces: graphCfg.sessionShadeWarmCacheMaxNamespaces
    };
  }
  ...
  function routeStretchRescueOptions() {
    return Object.assign({
      ...
      pedestrianRealmRescue: true, pedestrianRealmMarginM: 280, pedestrianRealmCorridorM: 200, pedestrianRealmMinImprovementM: 120,
      pedestrianRealmPortalSnapMaxM: 14, pedestrianRealmOpenBoundaryAttachMaxM: 7, pedestrianRealmOpenBoundaryBarrierClearanceM: 2.5,
      pedestrianRealmBoundarySampleM: 24, pedestrianRealmVisibilityBoundarySampleM: 34, pedestrianRealmMaxExplicitPortalsPerArea: 10, pedestrianRealmMaxOpenBoundaryPortals: 8,
      pedestrianRealmMaxBaseNodesPerArea: 18, pedestrianRealmMaxBoundaryVisibilityNodes: 14, pedestrianRealmCorridorVisibilitySampleM: 58, pedestrianRealmMaxCorridorVisibilityNodes: 10, pedestrianRealmObstacleBoundarySampleM: 28, pedestrianRealmMaxObstacleVisibilityNodes: 20, pedestrianRealmMaxVisibilityM: 220, pedestrianRealmVisibilityNeighborLimit: 8, pedestrianRealmMaxVisibilityEdgesPerArea: 260, pedestrianRealmVisibilitySampleM: 4, pedestrianRealmMaxSyntheticDistanceM: 480, pedestrianRealmMaxRouteToStraightRatio: 3.2,
    ...
    }, config.routeStretchRescue || {});
  }

  function graphFastestCandidate(candidates) {
    const list = (candidates || []).filter((c) => Array.isArray(c?.points) && c.points.length >= 2);
    return list.find((c) => c.kind === "graph-fastest") || list.reduce((best, c) => {
      const d = Number(c?.distanceM) || routeDistanceM(c.points || []);
      const bd = best ? (Number(best?.distanceM) || routeDistanceM(best.points || [])) : Infinity;
      return d < bd ? c : best;
    }, null);
  }

  function graphFastestDistanceM(candidates) {
    const c = graphFastestCandidate(candidates);
    return c ? (Number(c.distanceM) || routeDistanceM(c.points || [])) : Infinity;
  }

  function routeStretchAudit(a, b, candidates) {
    const opts = routeStretchRescueOptions();
    const A = asLatLng(a), B = asLatLng(b);
    const straightM = A && B ? haversineM(A, B) : 0;
    const routeM = graphFastestDistanceM(candidates);
    const ratio = straightM > 1 ? routeM / straightM : Infinity;
    const excessM = Number.isFinite(routeM) ? Math.max(0, routeM - straightM) : Infinity;
    const inScope = straightM >= Math.max(0, Number(opts.minStraightM) || 0) && straightM <= Math.max(1, Number(opts.maxStraightM) || 3000);
    const triggered = opts.enabled !== false && inScope && Number.isFinite(routeM) &&
      ratio >= Math.max(1, Number(opts.ratioThreshold) || 2.4) &&
      excessM >= Math.max(0, Number(opts.minExcessM) || 250);
    return {
      triggered, straightM, routeM, ratio, excessM, inScope,
      ratioThreshold: Math.max(1, Number(opts.ratioThreshold) || 2.4),
      minExcessM: Math.max(0, Number(opts.minExcessM) || 250)
    };
  }
  ...
  function candidateGeometryHash(points) {
  ...
  function withCandidateIdentity(candidate) {
    if (!candidate?.points?.length) return candidate;
    const geometryHash = candidate.geometryHash || candidateGeometryHash(candidate.points);
    return Object.assign({}, candidate, {
      geometryHash,
      stableCandidateId: candidate.stableCandidateId || `${candidate.id || candidate.kind || 'candidate'}:${geometryHash}`
    });
  }

  function geometryDuplicateEvidence(a, b) {
    const A = (a?.points || []).map(asLatLng).filter(Boolean);
    const B = (b?.points || []).map(asLatLng).filter(Boolean);
    if (A.length < 2 || B.length < 2) return { duplicate:false, reason:'missing-geometry' };
    const distanceDeltaM = Math.abs((Number(a.distanceM) || routeDistanceM(A)) - (Number(b.distanceM) || routeDistanceM(B)));
    if (distanceDeltaM > 8) return { duplicate:false, reason:'distance-delta', distanceDeltaM };
    if (haversineM(A[0], B[0]) > 4 || haversineM(A[A.length - 1], B[B.length - 1]) > 4) {
      return { duplicate:false, reason:'endpoint-delta', distanceDeltaM };
    }
    ...
    for (const [samples, target] of [[samplesA, B], [samplesB, A]]) {
      for (const point of samples) {
        ...
        maxOffsetM = Math.max(maxOffsetM, d); sumOffsetM += d; count += 1;
  ...
  function dedupeCandidates(candidates, audit = null) {
    ...
    for (const raw of candidates || []) {
      if (!raw?.points?.length) {
        audit?.dedupe?.push?.({ stage:'dedupe', status:'dropped', reason:'missing-geometry', candidateId:raw?.id || null });
        continue;
      }
      const candidate = withCandidateIdentity(raw);
      if (candidate.kind === 'manual' || candidate.kind === 'experimental-fused') {
        out.push(candidate);
        audit?.dedupe?.push?.({ stage:'dedupe', status:'kept', candidateId:candidate.id, stableCandidateId:candidate.stableCandidateId, geometryHash:candidate.geometryHash, reason:'protected-evidence-candidate' });
        continue;
      }
      let duplicateOf = null, proof = null;
      ...
      if (duplicateOf) {
        ...
        out.push(candidate);
        audit?.dedupe?.push?.({ stage:'dedupe', status:'kept', candidateId:candidate.id, stableCandidateId:candidate.stableCandidateId, geometryHash:candidate.geometryHash });
      }
    }
    return out;
  }
  ...
  // A candidate may disappear only with an explicit, positive explanation.
  function finalizeCandidateAudit(audit) {
    if (!audit) return null;
    const keyOf = (x) => x?.stableCandidateId || (x?.candidateId ? `id:${x.candidateId}` : null);
    const firstByKey = (items) => {
      const map = new Map();
    ...
    const dedupe = firstByKey(audit.dedupe);
    const quality = firstByKey(audit.quality);
    ...
    for (const generated of audit.generated || []) {
      ...
      if (displayed.has(key)) {
        result = { status:'displayed', reason:'displayed' };
      } else {
        const d = dedupe.get(key), q = quality.get(key), dt = detour.get(key), sc = scored.get(key);
        if (d?.status === 'deduped') {
          ...
          result = { status:'hidden', reason:'route-quality-rejected', details:q.reasons || [] };
        } else if (dt && dt.status !== 'eligible') {
          result = { status:'hidden', reason:dt.status };
        } else if (sc) {
          result = { status:'unexplained', reason:'scored-but-not-displayed' };
        } else {
          result = { status:'unexplained', reason:'candidate-vanished-without-lifecycle-reason' };
        }
      }
      const row = Object.assign({
        candidateId:generated.candidateId || null,
        stableCandidateId:generated.stableCandidateId || null,
        geometryHash:generated.geometryHash || null
      }, result);
      outcomes.push(row);
      if (row.status === 'unexplained') unexplained.push(row);
    }

    const graphHidden = [];
    for (const event of audit.graphLifecycle || []) {
      if (event?.status !== 'suppressed' && event?.status !== 'not-generated') continue;
      const explained = event.status === 'not-generated'
        ? Boolean(event.reason)
        : event.reason === 'exact-same-edge-sequence-as-fastest';
      const row = Object.assign({}, event, { explained });
      graphHidden.push(row);
      if (!explained) unexplained.push({
        candidateId:event.candidateId || null,
        stableCandidateId:event.stableCandidateId || null,
        geometryHash:event.geometryHash || null,
        status:'unexplained',
        reason:event.reason || 'graph-candidate-suppressed-without-proof'
      });
    }

    audit.accounting = {
      valid: unexplained.length === 0,
      generatedCount:(audit.generated || []).length,
      displayedCount:(audit.displayed || []).length,
      hiddenCount:outcomes.filter((x) => x.status === 'hidden').length + graphHidden.length,
      graphHiddenCount:graphHidden.length,
      unexplainedLossCount:unexplained.length,
      outcomes, graphHidden, unexplained
    };
  ...
  async function scoreCandidates(candidates, options = {}) {
    ...
    // dev35.3: dense final scoring is exact and candidate-independent, so run a
    // small number of candidates concurrently instead of serially paying the
    // full ShadeMap latency five times. Each analyzeRoute still preserves its
    // own ordered segment array and its existing per-route shade concurrency.
    // This changes scheduling only: geometry, timestamps, sample spacing,
    // aggregation, detour eligibility, and winner selection are untouched.
    const denseCandidateConcurrency = clamp(
      options.denseCandidateConcurrency,
      1,
      3,
      config.denseCandidateConcurrency ?? 2
    );
    const denseProgress = scoringPool.map(() => ({ done: 0, total: 0 }));
    let lastDenseProgressUiMs = 0;
    const updateDenseProgress = (index, done, total) => {
      denseProgress[index] = { done: Number(done) || 0, total: Number(total) || 0 };
      const now = nowMs();
      if (now - lastDenseProgressUiMs < 120 && done < total) return;
      lastDenseProgressUiMs = now;
      const finished = denseProgress.reduce((sum, p) => sum + Math.min(p.done, p.total || p.done), 0);
      const expected = denseProgress.reduce((sum, p) => sum + Math.max(p.total, p.done), 0);
    ...
    const scored = await runPool(
      ...
      async (candidate, i) => {
        ...
        const analysis = await analyzeRoute(candidate.points, {
          ...
          serial,
          includeHeat: false,
          onProgress: (done, total) => updateDenseProgress(i, done, total)
        });
        return Object.assign({}, candidate, { analysis, eligible: eligibleIds.has(candidate.id) });
      },
    ...
    return {
      ...
      denseScoring: {
        ...
        perCandidateShadeConcurrency: clamp(config.shadeConcurrency, 1, 6, 3),
        theoreticalMaxShadeConcurrency: denseCandidateConcurrency * clamp(config.shadeConcurrency, 1, 6, 3),
        candidateCount: scoringPool.length,
        sampleCount: scored.reduce((sum, c) => sum + Number(c?.analysis?.segments?.length || 0), 0)
      }
    };
  }
  ...
  function candidateName(candidate, bundle) {
    ...
    if (candidate?.kind === "topology-repair-shade") return "OSM 端點拓樸修復最不曬候選";
    if (candidate?.kind === "interior-repair-fastest") return "OSM 交叉點修復最快候選";
    if (candidate?.kind === "interior-repair-shade") return "OSM 交叉點修復最不曬候選";
    ...
    if (candidate?.kind === "pedestrian-realm-shade") return "公園／開放行人空間最不曬候選";
    ...
    if (candidate?.kind === "graph-fastest") {
      if (candidate?.graphMeta?.usesConditionalPrivateAccess) return "條件式園區步行最快";
      return candidate?.graphMeta?.backend === "nationwide-hgr2" ? "全臺步行路網最快" : (candidate?.graphMeta?.backend === "nationwide-hgr1" ? "全臺圖資 Graph 最快" : "OSM Graph 最快");
    }
  ...
  function activeEvidenceRouteLines(bundle,a,b) {
    const lines=(bundle?.scored||[]).filter((c)=>c?.kind!=='manual'&&Array.isArray(c?.points)&&c.points.length>=2).map((c)=>c.points);
    if(!lines.length&&a&&b)lines.push([a,b]);
    return lines;
  }
  ...
  function nationwideTilesStatusHtml() {
    ...
    if (!api) return '<p class="re-fusion-note"><strong>Nationwide tiles：</strong>loader 未載入；目前仍可使用既有 AOI evidence。</p>';
    const st = api.getState?.() || {};
    const last = st.lastLoad || lastNationwideTileLoad;
    const graphLast = st.lastGraphLoad || lastNationwideGraphLoad;
    if (st.status === 'error') return `<p class="re-fusion-note"><strong>Nationwide tiles：</strong>載入錯誤 ${escapeHtml(st.error || 'unknown')}；production routing 可回退 Overpass。</p>`;
    if (!last && !graphLast) return `<p class="re-fusion-note"><strong>Nationwide tiles：</strong>${st.manifestLoaded ? `manifest ready · 全國 index ${Number(st.tileCount || 0)} tiles` : '尚未載入 manifest'}；A→B 時只會 lazy-load 路線附近 tile。</p>`;
    ...
    const graphBackend = graphLast?.backend === 'nationwide-hgr2' ? 'HGR2' : 'HGR1';
    const graphUnit = graphBackend === 'HGR2' ? 'fine edges' : 'directed edges';
    const graphText = graphLast ? `${graphBackend} graph ${Number(graphLast.loadedTileCount || 0)} tiles／${Number(graphLast.nodeCount || 0)} nodes／${Number(graphLast.edgeCount || 0)} ${graphUnit}` : 'graph 尚未載入';
    const graphCache = Number(st.cachedGraph2TileCount || 0) ? `${Number(st.cachedGraph2TileCount || 0)} HGR2` : `${Number(st.cachedGraphTileCount || 0)} HGR1`;
    return `<p class="re-fusion-note"><strong>Nationwide tiles：</strong>${evidenceText}；${graphText}；cache ${Number(st.cachedTileCount || 0)} evidence／${graphCache} graph tiles。全國原始 ZIP 未進瀏覽器。</p>`;
  }
  ...
  function multiSourcePanelHtml() {
    ...
    if (!api) return '<div class="re-multisource-note" data-re-multisource><b>Multi-source Evidence</b><span>模組未載入；production graph 未修改。</span></div>';
    const st = api.getState();
    if (st.status === 'idle') {
      return '<div class="re-multisource-note" data-re-multisource><b>Multi-source Evidence</b><span>OSM 仍是 production base graph；外部來源只作 provenance/evidence，不做 proximity 自動補橋。</span><div class="re-multisource-actions"><button type="button" data-re-multisource-load>載入多來源證據矩陣</button></div></div>';
    }
    if (st.status === 'loading') {
      return '<div class="re-multisource-note" data-re-multisource><b>Multi-source Evidence</b><span>正在讀取預處理 AOI evidence index…</span></div>';
    }
    if (st.status === 'error') {
      return `<div class="re-multisource-note is-error" data-re-multisource><b>Multi-source Evidence</b><span>讀取失敗：${escapeHtml(st.error || 'unknown')}</span><div class="re-multisource-actions"><button type="button" data-re-multisource-load>重試</button></div></div>`;
    }

    // dev36.1: the bundled dev25–dev32 Haidian gap index is a regression fixture,
    // not evidence for every route in Taiwan. Once a live A→B exists, show that
    // static index only if the active route is geographically near it; otherwise
    // show the current route-local discovery result (including a clean zero-gap state).
    const activeRouteLines=activeEvidenceRouteLines(lastCandidateBundle,aPoint,bPoint);
    const hasActiveRoute=activeRouteLines.length>0;
    const knownIndexApplies = !hasActiveRoute || routeNearKnownEvidenceIndex(st.evidenceIndex, activeRouteLines, 700);
    const currentDiscovery = lastOfficialDiscovery || lastCandidateBundle?.officialEvidenceDiscovery || null;
    const displayIndex = knownIndexApplies
      ? st.evidenceIndex
      : (currentDiscovery?.evidenceIndex || { summary:{ gapCount:0, verified:0, manualReview:0, rejected:0, routableVerified:0 }, gaps:[] });
    const routeScopeNote = !hasActiveRoute ? '<p class="re-fusion-note">Developer fixture：目前沒有A/B；以下bundled海佃資料不是任何新路線的通行證據。</p>' : knownIndexApplies ? '' : '<p class="re-fusion-note"><strong>dev36 route scope：</strong>已隱藏不屬於目前 A→B 的 bundled 海佃 regression gap；以下只顯示本次 route-local discovery。</p>';
    ...
    const gaps = (displayIndex?.gaps || []).map((gap) => {
      const rows = [
      ...
      return `<details class="re-evidence-gap"><summary>${escapeHtml(gap.id || 'gap')} · ${Number(gap.geometryGapM || 0).toFixed(1)}m · <b>${escapeHtml(gap.decision || 'unknown')}</b></summary><div class="re-evidence-matrix">${rows}</div><p>${escapeHtml(gap.decisionReason || '')}</p><p><strong>productionAllowed=false</strong> · ${escapeHtml(gap.productionReason || 'production graph safety lock')}</p></details>`;
    }).join('');

    const summary = displayIndex?.summary || {};
    const fusion = knownIndexApplies
      ? (st.fusionPlan || {})
      : { verifiedCandidateCount:Number(currentDiscovery?.verifiedGapCount || 0), routableWitnessCount:Number(currentDiscovery?.verifiedGapCount || 0) };
    const fusionApi = window.HaidianExperimentalFusionRouter;
    const fusionState = fusionApi?.getState?.() || null;
    const lastRun = lastCandidateBundle ? (lastCandidateBundle?.experimentalFusion?.result || null) : (fusionState?.lastRun || null);
    ...
    const fusionButton = fusionApi
      ...
      : '';
    const witnessNotes = (displayIndex?.gaps || []).map((gap) => {
    ...
    const auto = currentDiscovery || (!hasActiveRoute?window.HaidianOfficialEvidenceDiscovery?.getState?.()?.lastDiscovery:null) || null;
  ...
  async function loadMultiSourceEvidence() {
    const api = window.HaidianMultiSourceEvidence;
    if (!api) return setStatus('multi-source evidence 模組未載入。', 'warning');
    refreshMultiSourcePanel();
    ...
    if (result.status === 'ready') {
      ...
      setStatus(u ? `證據矩陣已載入；目前仍有 ${u} 個 gap 含未綁定來源，這些是未知，不是負證據。` : '證據矩陣已載入；production graph 保持不變。', u ? 'warning' : 'ok');
    } else {
  ...
  function candidateCorrectnessSummaryHtml(bundle) {
    const d = bundle?.graphDiagnostics || null;
    const audit = bundle?.candidateAudit || null;
    if (!d && !audit) return '';
    const graphLifecycle = Array.isArray(d?.candidateLifecycle) ? d.candidateLifecycle : [];
    const suppressed = graphLifecycle.filter((x) => x?.status === 'suppressed');
    const collisions = graphLifecycle.filter((x) => x?.legacyRouteSignatureCollision === true);
    const prune = d?.pruningCertificate || null;
    const replay = d?.exactReplay || null;
    const deduped = (audit?.dedupe || []).filter((x) => x?.status === 'deduped');
    const generated = (audit?.generated || []).length;
    const displayed = (audit?.displayed || []).length;
    const rows = [];
    if (generated || displayed) rows.push(`candidate lifecycle：generated ${generated} → displayed ${displayed}；UI dedupe ${deduped.length}`);
    if (audit?.accounting) rows.push(`Loss accounting：${audit.accounting.valid ? 'PASS' : 'FAIL'}；unexplained ${Number(audit.accounting.unexplainedLossCount || 0)}；graph hidden ${Number(audit.accounting.graphHiddenCount || 0)}`);
    if (graphLifecycle.length) rows.push(`Graph candidates：${graphLifecycle.length} 個生成事件；suppressed ${suppressed.length}${collisions.length ? `；偵測到 ${collisions.length} 個舊 routeSignature collision` : ''}`);
    if (prune) rows.push(`Pruning proof：${prune.valid ? 'PASS' : 'FAIL'}；kept ${Number(prune.keptEdges || 0)} / omitted ${Number(prune.omittedEdges || 0)}；違反 ${Number(prune.violationCount || 0)}`);
    if (replay?.available) rows.push(`Exact replay：temporal ${formatMinutes(replay.estimatedDirectSunSeconds || 0)} → exact ${formatMinutes(replay.directSunSeconds || 0)}；Δ ${Number(replay.errorSeconds || 0).toFixed(1)}s；${replay.withinTolerance ? 'within tolerance' : 'OUTSIDE tolerance'}`);
    const live = lastCandidateCorrectnessAudit;
    if (live?.available) rows.push(`A/B audit：temporal ${Number(live.temporal?.candidateCount || 0)} vs on-demand ${Number(live.onDemand?.candidateCount ?? live.exact?.candidateCount ?? 0)}；fastest geometry ${live.comparison?.sameFastestGeometry ? 'same' : 'DIFF'}；min-sun geometry ${live.comparison?.sameMinSunGeometry ? 'same' : 'DIFF'}；pruning ${live.pruningValid ? 'PASS' : 'FAIL'}`);
    const details = rows.map((x) => `<span>${escapeHtml(x)}</span>`).join('');
  ...
  function nationwideRegressionSummaryHtml() {
    ...
    if (busy) body = '<span>正在跑北／中／南／東與 safety regression；不修改目前 production graph。</span>';
      ...
      const caseDetails = (run.results || []).map((r) => {
        const failed = (r.checks || []).filter((c) => c?.severity === 'required' && !c?.pass);
        const graphStage = r.graph?.stage ? `stage ${r.graph.stage} / ${Math.round(Number(r.graph.marginM || 0))}m / ring ${Number(r.graph.ring || 0)}` : '';
        const graphShape = r.graph?.available !== false && r.graph?.backend ? `${r.graph.backend}；${Number(r.graph.loadedTileCount || 0)} tiles；${Number(r.graph.nodes || 0)} nodes／${Number(r.graph.edges || 0)} edges` : 'graph unavailable';
        const headline = failed.length ? failed.map((c) => `${c.name}: ${c.detail || 'FAIL'}`).join('；') : `${graphStage}${graphStage ? '；' : ''}${graphShape}`;
        const checks = (r.checks || []).map((c) => `<div><strong>${c.pass ? 'PASS' : 'FAIL'} ${escapeHtml(c.name || 'check')}</strong><span>${escapeHtml(c.detail == null ? '' : String(c.detail))}</span></div>`).join('');
        return `<details class="re-evidence-gap"><summary>${r.pass ? 'PASS' : 'FAIL'} ${escapeHtml(r.label || r.id)} · ${escapeHtml(headline)}</summary><div class="re-evidence-matrix">${checks}</div></details>`;
      }).join('');
      body = `<span>required ${run.requiredPass ? 'PASS' : 'FAIL'}；${Number(run.passCount || 0)}/${Number(run.caseCount || 0)} cases pass；${escapeHtml(cases)}。</span><span>productionGraphMutated=false；normalRouteRuntimeModified=false。</span>${caseDetails}`;
  ...
  async function runCandidateCorrectnessAuditLive() {
    ...
    if (!api?.runCandidateCorrectnessAudit || !aPoint || !bPoint) {
      setStatus('目前沒有可執行 dev32 candidate correctness audit 的 A/B graph。', 'warning');
      return;
    }
    try {
      setStatus('dev32：正在同一份 graph 上執行 temporal-table ↔ on-demand A/B correctness audit…', 'loading');
      const result = await api.runCandidateCorrectnessAudit(aPoint, bPoint, null, {
        departure: departureDateFromPanel(),
        speedMps: speedMpsFromPanel(),
        detourPct: detourCapFromPanel(),
      ...
      lastCandidateCorrectnessAudit = result;
      if (lastCandidateBundle) renderCandidateBundle(lastCandidateBundle, { fit:false });
      if (!result?.available) {
        setStatus(`dev32 correctness audit 無法執行：${result?.reason || 'unknown'}`, 'warning');
        return;
      }
      const ok = result.comparison?.sameCandidateCount && result.comparison?.sameFastestGeometry && result.comparison?.sameMinSunGeometry && result.pruningValid;
      const onDemandCount = result.onDemand?.candidateCount ?? result.exact?.candidateCount ?? 0;
      setStatus(`dev32 audit 完成：temporal ${result.temporal?.candidateCount || 0} 條 / on-demand ${onDemandCount} 條；fastest ${result.comparison?.sameFastestGeometry ? '一致' : '不同'}；min-sun ${result.comparison?.sameMinSunGeometry ? '一致' : '不同'}；pruning ${result.pruningValid ? 'PASS' : 'FAIL'}。`, ok ? 'ok' : 'warning');
    } catch (error) {
      setStatus(`dev32 correctness audit 失敗：${error?.message || error}`, 'warning');
    }
  }
  ...
  function candidatesHtml(bundle) {
    ...
    const rows = bundle.scored.map((c) => {
      ...
      if (c.kind === 'topology-repair-fastest' || c.kind === 'topology-repair-shade') badges.push('<em class="graph">OSM 端點修復</em>');
    ...
    let unresolvedStretchText = stretchState?.earlyLiveProbe
      ? ' 系統已因「短距離但極端繞遠」停止大範圍 HGR2 擴張，依序檢查 focused live OSM、局部 noding／跨來源線網，再用 Pedestrian Realm Graph 檢查公園／廣場面狀行人空間；若仍只有長繞路，才視為資料尚未證實短連通。'
      : ' 系統已擴大靜態 graph 並交叉檢查 HGR1 / live OSM，再掃描短 A→B corridor；若仍只有長繞路，應視為「資料尚未證實短連通」，而不是可靠的最短步行答案。';
    if(stretchState?.pedestrianRealmAttempted&&!stretchState?.pedestrianRealmAccepted){
      const nearby=(stretchState.pedestrianRealmAreaCoverage||[]).find((a)=>a.kind==='public-park') || (stretchState.pedestrianRealmAreaCoverage||[])[0];
      const endpoint=stretchState.pedestrianRealmEndpointCoverage||{};
      if(stretchState.pedestrianRealmReason==='unmapped-terminal-access'){
        unresolvedStretchText+=` OSM 已找到 ${Math.round(Number(stretchState.pedestrianRealmAreaCount||0))} 個面狀空間，但 A／B 距可用道路分別約 ${Number(endpoint.aRoadGapM||0).toFixed(1)}／${Number(endpoint.bRoadGapM||0).toFixed(1)}m${nearby?`、B 距所列公園邊界約 ${Number(nearby.bBoundaryM||0).toFixed(1)}m`:''}；端點銜接缺少可驗證的通行幾何或遭障礙物阻擋，長繞路不可當作可信最短路。`;
      } else if(stretchState.pedestrianRealmReason==='incomplete-obstacle-geometry'){
        unresolvedStretchText+=' OSM 水體／建物／圍籬幾何不完整；已停止面狀捷徑，不能據此判定最短路。';
      } else if(stretchState.pedestrianRealmReason==='realm-source-unavailable'){
        unresolvedStretchText+=' OSM 面狀資料讀取失敗；目前沒有證據足以判定公園入口或園內最短路。';
      } else if(stretchState.pedestrianRealmReason==='no-auto-eligible-realm'||stretchState.pedestrianRealmReason==='auto-realm-outside-corridor'){
        unresolvedStretchText+=' 來源資料沒有覆蓋 A→B 走廊的可自動通行公園／廣場面，仍須補足面狀或入口來源。';
      }
    }
    const routeStretchDetail = stretchState?.localNodingAccepted
      ? ` strict live OSM 裡找到高可信度的「端點幾何相接但沒有 shared node」斷點（gap 約 ${Number(stretchState.localNodingGapM||0).toFixed(1)}m）；只在 detached clone 修復。`
      : stretchState?.interiorNodingAccepted
        ? ` strict live OSM 裡找到同層 edge↔edge 幾何交叉但沒有 shared node 的 interior noding defect；只在 detached clone 建立交叉點。`
        : stretchState?.crossSourceAccepted
          ? ` 單一資料源都不完整，但 detached OSM × Overture corridor union 以 ${Math.round(Number(stretchState.crossSourceConnectorCount||0))} 個 tiny source-backed stitch 救回較短路線（最大 gap 約 ${Number(stretchState.crossSourceGapMaxM||0).toFixed(1)}m）；兩份原始 graph 均未修改。`
          : stretchState?.pedestrianRealmAccepted
            ? ` line graph 本身沒有短連通，但 public Pedestrian Realm Graph 找到約 ${Math.round(Number(stretchState.pedestrianRealmDistanceM||0))}m 的面狀行人候選；使用 ${Math.round(Number(stretchState.pedestrianRealmAutoAreaCount||0))} 個 public realm、${Math.round(Number(stretchState.pedestrianRealmPortalCount||0))} 個入口／開放邊界 anchor，並把水體、建物、圍籬視為障礙。約 ${Math.round(Number(stretchState.pedestrianRealmSyntheticDistanceM||0))}m 並非來源中明確畫出的步道，因此需依現場步道確認。`
            : stretchState?.conditionalAccessAccepted
            ? ` 已找到一條明顯較短的「條件式通行」路線：其中約 ${Math.round(Number(stretchState.conditionalAccessPrivateDistanceM||0))}m 在 OSM 標記為 access=private，但 private 段只出現在 A/B 終點附近；請依現場入口、開放時間或許可規則確認是否可通行。`
            : stretchState?.valhallaAccepted
              ? ' 獨立 Valhalla pedestrian engine 找到明顯較短的交叉驗證候選；它會以獨立候選顯示，不會修改 production graph。'
              : bundle.experimentalFusionStatus?.available
                ? ' 已找到可比較的 verified official-fusion rescue 候選。'
                : unresolvedStretchText;
    const routeStretchWarning = stretchState?.triggered
      ? `<div class="re-candidate-success" style="border-color:#f59e0b;background:#fffbeb;color:#78350f"><b>Route Stretch Rescue 已觸發</b><span>A、B 的已知 graph 最快路線曾達 ${Math.round(Number(stretchState.initialRouteM||0))} m，而直線約 ${Math.round(Number(stretchState.initialStraightM||0))} m（${Number(stretchState.initialRatio||0).toFixed(1)}×）。${routeStretchDetail}</span></div>`
      : '';
    ...
    if (graphDiag) {
      ...
      const prunePct = rawEdges > 0 ? Math.max(0, (1 - pruned / rawEdges) * 100) : 0;
      ...
      const graphBreakdown = hgr2Backend
        ...
        : hgr1Backend
          ...
          : '';
      const perfText = nationwideBackend ? `tile ${(Number(tp.totalMs||0)/1000).toFixed(1)}s・graph ${(Number(p.totalMs||0)/1000).toFixed(1)}s${graphBreakdown}` : `graph ${(Number(p.totalMs||0)/1000).toFixed(1)}s`;
      const snapLabel = nationwideBackend ? 'A、B edge 吸附' : 'A、B 行人優先 edge 吸附';
      const snapPlan = graphDiag.connectivitySnapPlan || null;
      const snapRescueText = snapPlan?.fallbackUsed
        ? `；connectivity snap rescue：最近 edge ${Number(snapPlan.nearestA||0).toFixed(1)}/${Number(snapPlan.nearestB||0).toFixed(1)} m → 共通 component ${Number(snapPlan.selectedA||0).toFixed(1)}/${Number(snapPlan.selectedB||0).toFixed(1)} m（${Math.round(Number(snapPlan.componentNodeCount||0))} nodes／${Math.round(Number(snapPlan.componentEdgeCount||0))} edges）`
        : '';
    ...
    return `<section class="re-candidates">
      ...
      ${bundle.performance ? (() => { const sd=bundle.performance.shadeEngine||{}; const bi=sd.buildingSpatialIndex||{}; const red=Number.isFinite(Number(bi.reductionRatio)) ? `${(Number(bi.reductionRatio)*100).toFixed(1)}%` : '—'; return `<div class="re-method-note"><b>dev37.1 Performance：</b>總計 ${(Number(bundle.performance.totalMs||0)/1000).toFixed(1)}s；graph ${(Number(bundle.performance.graphMs||0)/1000).toFixed(1)}s；fusion ${(Number(bundle.performance.fusionMs||0)/1000).toFixed(1)}s；dense ${(Number(bundle.performance.denseScoreMs||0)/1000).toFixed(1)}s；provider ${(Number(bundle.performance.providerMs||0)/1000).toFixed(1)}s。${bundle.denseScoring ? `<br>Dense pool：candidate ${Math.round(Number(bundle.denseScoring.candidateConcurrency||1))} × shade ${Math.round(Number(bundle.denseScoring.perCandidateShadeConcurrency||1))}；上限 ${Math.round(Number(bundle.denseScoring.theoreticalMaxShadeConcurrency||1))} concurrent samples；${Math.round(Number(bundle.denseScoring.sampleCount||0))} samples。` : ''}<br>Shade engine：building ${(Number(sd.buildingEvalMs||0)/1000).toFixed(1)}s；canopy ${(Number(sd.canopyEvalMs||0)/1000).toFixed(1)}s；building broad-phase 裁掉 ${red}；平均候選 ${Number(bi.averageCandidates||0).toFixed(1)}/${Math.round(Number(sd.buildingFeatureCount||0))}；shared graph cache ${Math.round(Number(bundle.performance.sharedGraphShadeCacheSize||0))}。${bundle.graphDiagnostics?.temporalShadeTable ? `<br>Temporal table：新算 ${Math.round(Number(bundle.graphDiagnostics.temporalShadeTable.evaluated||0))}；warm/table hits ${Math.round(Number(bundle.graphDiagnostics.temporalShadeTable.cacheHits||0))}；prewarm ${(Number(bundle.graphDiagnostics.performance?.temporalShadeTableMs||0)/1000).toFixed(1)}s；search misses ${Math.round(Number(bundle.graphDiagnostics.shadeEdgeEvaluations||0))}；search cache hits ${Math.round(Number(bundle.graphDiagnostics.shadeCacheHits||0))}。` : ''}${bundle.performance.shadeWarmCache ? `<br>Session shade warm cache：${bundle.performance.shadeWarmCache.lateAcquired ? 'LATE-CAPTURE' : (bundle.performance.shadeWarmCache.enabled ? 'ON' : 'OFF')}；seed ${Math.round(Number(bundle.performance.shadeWarmCache.seeded||0))}；persist ${Math.round(Number(bundle.performance.shadeWarmCache.persisted||0))}；reject ${Math.round(Number(bundle.performance.shadeWarmCache.rejected||0))}；session ${Math.round(Number(bundle.performance.shadeWarmCache.namespaceEntriesAfter||bundle.performance.shadeWarmCache.namespaceEntriesBefore||bundle.performance.sharedGraphShadeCacheSize||0))}；ns ${Math.round(Number(bundle.performance.shadeWarmCache.namespaceCount||0))} / total ${Math.round(Number(bundle.performance.shadeWarmCache.totalEntries||0))}${bundle.performance.shadeWarmCache.lateAcquired ? '；本次搜尋起始未 seed，結束時已安全寫回供下一次重用' : ''}${bundle.performance.shadeWarmCache.commitSkipped ? `；未寫回（${escapeHtml(bundle.performance.shadeWarmCache.commitReason||'context changed')}）` : ''}。` : ''}${bundle.performance.routeStretchRescue?.triggered ? `<br>Route Stretch Rescue：initial ${Math.round(Number(bundle.performance.routeStretchRescue.initialRouteM||0))}m / straight ${Math.round(Number(bundle.performance.routeStretchRescue.initialStraightM||0))}m（${Number(bundle.performance.routeStretchRescue.initialRatio||0).toFixed(1)}×）；stage-expand ${bundle.performance.routeStretchRescue.nationwideStageExpansion?'yes':'no'}；Overpass ${bundle.performance.routeStretchRescue.overpassAttempted?(bundle.performance.routeStretchRescue.overpassAccepted?'accepted':'checked'):'no'}；local-noding ${bundle.performance.routeStretchRescue.localNodingAttempted?(bundle.performance.routeStretchRescue.localNodingAccepted?'accepted':'checked'):'no'}${bundle.performance.routeStretchRescue.localNodingAttempted&&Number.isFinite(Number(bundle.performance.routeStretchRescue.localNodingDistanceM))?`/${Math.round(Number(bundle.performance.routeStretchRescue.localNodingDistanceM))}m`:''}${bundle.performance.routeStretchRescue.localNodingAttempted?` gap ${Number(bundle.performance.routeStretchRescue.localNodingGapM||0).toFixed(1)}m cand ${Math.round(Number(bundle.performance.routeStretchRescue.localNodingCandidateCount||0))}/auto ${Math.round(Number(bundle.performance.routeStretchRescue.localNodingAutoEligibleCount||0))}/review ${Math.round(Number(bundle.performance.routeStretchRescue.localNodingReviewOnlyCount||0))}/tested ${Math.round(Number(bundle.performance.routeStretchRescue.localNodingTestedCount||0))}`:''}；interior ${bundle.performance.routeStretchRescue.interiorNodingAttempted?(bundle.performance.routeStretchRescue.interiorNodingAccepted?'accepted':'checked'):'no'}${bundle.performance.routeStretchRescue.interiorNodingAttempted&&Number.isFinite(Number(bundle.performance.routeStretchRescue.interiorNodingDistanceM))?`/${Math.round(Number(bundle.performance.routeStretchRescue.interiorNodingDistanceM))}m`:''}${bundle.performance.routeStretchRescue.interiorNodingAttempted?` cand ${Math.round(Number(bundle.performance.routeStretchRescue.interiorNodingCandidateCount||0))}/tested ${Math.round(Number(bundle.performance.routeStretchRescue.interiorNodingTestedCount||0))}`:''}；xsrc ${bundle.performance.routeStretchRescue.crossSourceAttempted?(bundle.performance.routeStretchRescue.crossSourceAccepted?'accepted':'checked'):'no'}${bundle.performance.routeStretchRescue.crossSourceAttempted&&Number.isFinite(Number(bundle.performance.routeStretchRescue.crossSourceDistanceM))?`/${Math.round(Number(bundle.performance.routeStretchRescue.crossSourceDistanceM))}m`:''}${bundle.performance.routeStretchRescue.crossSourceAttempted?` cand ${Math.round(Number(bundle.performance.routeStretchRescue.crossSourceCandidateCount||0))}/auto ${Math.round(Number(bundle.performance.routeStretchRescue.crossSourceAutoEligibleCount||0))}/review ${Math.round(Number(bundle.performance.routeStretchRescue.crossSourceReviewOnlyCount||0))}/tested ${Math.round(Number(bundle.performance.routeStretchRescue.crossSourceTestedCount||0))} stitches ${Math.round(Number(bundle.performance.routeStretchRescue.crossSourceConnectorCount||0))} max-gap ${Number(bundle.performance.routeStretchRescue.crossSourceGapMaxM||0).toFixed(1)}m`:''}；realm ${bundle.performance.routeStretchRescue.pedestrianRealmAttempted?(bundle.performance.routeStretchRescue.pedestrianRealmAccepted?'accepted':'checked'):'no'}${bundle.performance.routeStretchRescue.pedestrianRealmAttempted&&Number.isFinite(Number(bundle.performance.routeStretchRescue.pedestrianRealmDistanceM))?`/${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmDistanceM))}m`:''}${bundle.performance.routeStretchRescue.pedestrianRealmAttempted?` areas ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmAutoAreaCount||0))}/${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmAreaCount||0))} review ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmReviewAreaCount||0))} obstacles ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmObstacleCount||0))} portals ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmPortalCount||0))} (explicit ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmExplicitPortalCount||0))}/open ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmOpenBoundaryPortalCount||0))}) vis ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmVisibilityEdgeCount||0))} synth ${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmSyntheticDistanceM||0))}m/${Math.round(Number(bundle.performance.routeStretchRescue.pedestrianRealmSyntheticRatio||0)*100)}%`:''}；conditional ${bundle.performance.routeStretchRescue.conditionalAccessAttempted?(bundle.performance.routeStretchRescue.conditionalAccessAccepted?'accepted':'checked'):'no'}${bundle.performance.routeStretchRescue.conditionalAccessAttempted?`/${Math.round(Number(bundle.performance.routeStretchRescue.conditionalAccessDistanceM||0))}m`:''}；Valhalla ${bundle.performance.routeStretchRescue.valhallaAttempted?(bundle.performance.routeStretchRescue.valhallaAccepted?'accepted':'checked'):'no'}${bundle.performance.routeStretchRescue.valhallaAttempted&&Number.isFinite(Number(bundle.performance.routeStretchRescue.valhallaDistanceM))?`/${Math.round(Number(bundle.performance.routeStretchRescue.valhallaDistanceM))}m`:''}；direct evidence ${bundle.performance.routeStretchRescue.directEvidenceProbe?'yes':'no'}；final ${Math.round(Number(bundle.performance.routeStretchRescue.finalRouteM||bundle.performance.routeStretchRescue.initialRouteM||0))}m（${Number(bundle.performance.routeStretchRescue.finalRatio||bundle.performance.routeStretchRescue.initialRatio||0).toFixed(1)}×）。` : ''}</div>`; })() : ''}
      ${bundle.performance?.realmOpportunity?.attempted ? (()=>{const r=bundle.performance.realmOpportunity;return `<div class="re-method-note"><b>Realm 遮蔭機會：</b>${r.accepted?'已加入候選':'未加入候選'}；${escapeHtml(r.reason||'public-realm')}；面 ${Number(r.autoAreaCount||0)}/${Number(r.areaCount||0)}；來源 ${(Number(r.sourceMs||0)/1000).toFixed(1)}s${r.cacheHit?'（cache）':''}／計算 ${(Number(r.computeMs||0)/1000).toFixed(1)}s；shade search ${r.shadeSearch?.complete?'完成':'未完成'}${r.shadeSearch?.reason?`（${escapeHtml(r.shadeSearch.reason)}）`:''}；${Number(r.candidateCount||0)}候選；${Number(r.shadeEdgeEvaluations||0)} edge評估。${r.shadeSearch?.profile?` ${Number(r.shadeSearch.profile.expandedStates||0)} 狀態；採樣 ${Number(r.shadeSearch.profile.modelCallsCompleted||0)}/${Number(r.shadeSearch.profile.modelCallsStarted||0)} 完成、${Number(r.shadeSearch.profile.modelCallsPending||0)} 處理中；${escapeHtml(r.shadeSearch.profile.termination||'')}。`:''}${(r.sourceErrors||[]).length?escapeHtml(r.sourceErrors.join(' | ')):''} ${r.endpointCoverage?`A/B路網gap ${Number(r.endpointCoverage.aRoadGapM||0).toFixed(1)}/${Number(r.endpointCoverage.bRoadGapM||0).toFixed(1)}m。`:''} 未完成shade search時只保留最快候選，不宣稱全域最不曬。</div>`;})():''}
      ${bundle.performance?.routeStretchRescue?.pedestrianRealmAttempted ? (()=>{const r=bundle.performance.routeStretchRescue,d=r.pedestrianRealmFeatureCounts||{},ec=r.pedestrianRealmEndpointCoverage||{},ac=(r.pedestrianRealmAreaCoverage||[]).find((a)=>a.kind==='public-park')||(r.pedestrianRealmAreaCoverage||[])[0],kinds=Object.entries(d.sourceTagKinds||{}).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([k,v])=>`${escapeHtml(k)}:${v}`).join(', ')||'—';return `<div class="re-method-note"><b>Realm OSM 來源診斷：</b>${escapeHtml(r.pedestrianRealmSource||'未取得')}；判定 ${escapeHtml(r.pedestrianRealmReason||(r.pedestrianRealmAccepted?'accepted':'checked'))}；元素 ${Math.round(Number(d.elementCount||0))}、ways ${Math.round(Number(d.wayCount||0))}、relations ${Math.round(Number(d.relationCount||0))}；tags ${kinds}；relation 缺 member ${Math.round(Number(d.relationMissingMemberWays||0))}／未閉合 ${Math.round(Number(d.relationUnclosedRings||0))}／nested ${Math.round(Number(d.relationNestedMembers||0))}；補齊 ${Math.round(Number(r.pedestrianRealmRelationCompletion?.completed||0))}/${Math.round(Number(r.pedestrianRealmRelationCompletion?.needed||0))}；containment ${r.pedestrianRealmContainmentAttempted?'查過':'未觸發'}。${ec.bRoadGapM!=null?` B→路網 ${Number(ec.bRoadGapM).toFixed(1)}m${ac?`，B→${escapeHtml(ac.kind)}邊界 ${Number(ac.bBoundaryM||0).toFixed(1)}m（${ac.bInside?'內':'外'}）`:''}；A→路網 ${Number(ec.aRoadGapM||0).toFixed(1)}m。`:''}${(r.pedestrianRealmSourceErrors||[]).length?` 來源錯誤：${escapeHtml(r.pedestrianRealmSourceErrors.join(' | '))}。`:''}${r.pedestrianRealmError?` 錯誤：${escapeHtml(r.pedestrianRealmError)}。`:''}</div>`;})():''}
  ...
  async function analyzeAB() {
    ...
    let shadeWarmCacheHandle = null;
    let shadeWarmCacheModelToken = null;
    try {
      try { window.HaidianShade?.resetRouteDiagnostics?.(); } catch (_) {}
      ...
      setStatus("正在準備 A→B 候選；dev37.1 已啟用 Route Stretch + Pedestrian Realm Graph Rescue，並保留 Local/Interior Noding + OSM×Overture locks…", "loading");
      ...
      // A bounded source prefetch runs beside ordinary graph loading. It never
      ...
      let stretchRescueCandidates = [];
      let selectedNationwideFullPlan = null;
      let runSelectedNationwideFullSearch = null;
      // dev36.3: keep the best stretched HGR graph available for a detached
      // OSM×Overture corridor-union rescue after strict live OSM is fetched.
      let stretchRescueNationwideGraph = null;
      let earlyLiveStretchRescue = false;
      ...
      if (config.graphRouting?.enabled !== false && window.HaidianPedestrianGraph?.findRoutes) {
        ...
        try {
          ...
          if (window.HaidianPedestrianGraph?.acquireSessionShadeWarmCache) {
            ...
            perf.shadeWarmCache = {
              ...
              namespaceEntriesBefore: Number(shadeWarmCacheHandle?.namespaceEntriesBefore || 0),
              namespaceCount: Number(shadeWarmCacheHandle?.namespaceCount || 0),
              totalEntries: Number(shadeWarmCacheHandle?.totalEntries || 0),
              modelContext: warmContext.modelContext || null,
              coverageToken: String(warmContext.modelContext?.coverageToken || '')
            };
          }
        } catch (warmError) {
        ...
        if (config.nationwideTiles?.enabled !== false && config.nationwideTiles?.preferGraphRouting !== false && window.HaidianPedestrianGraph?.findRoutesOnExternalGraph) {
          ...
          const routeLoadedNationwideStage = async (loaded, attempt, stage, fastestOnly = false) => {
            ...
            attempt.topologyOnly = fastestOnly === true;
          ...
          const rescueOpts = routeStretchRescueOptions();
          const topologyProbeFastestOnly = rescueOpts.topologyProbeFastestOnly !== false;
          const shouldJumpToFocusedLiveRescue = (audit) => Boolean(
            topologyProbeFastestOnly && audit?.triggered &&
            Number(audit.ratio || 0) >= Math.max(2.5, Number(rescueOpts.earlyLiveProbeRatioThreshold) || 4.0) &&
            Number(audit.straightM || Infinity) <= Math.max(100, Number(rescueOpts.earlyLiveProbeMaxStraightM) || 900)
          );
          let bestStretchedNationwide = null;
          let acceptedNationwide = false;
          let hgr1StretchCrossChecked = false;
          const rememberStageRoute = (candidates, result, attempt, loaded, stage) => {
            const audit = routeStretchAudit(aPoint, bPoint, candidates);
            attempt.routeStretch = audit;
            if (!candidates.length) return { accept:false, suspicious:false, audit };
            const d = graphFastestDistanceM(candidates);
            if (!bestStretchedNationwide || d < bestStretchedNationwide.distanceM) {
              bestStretchedNationwide = { candidates: candidates.slice(), result, distanceM:d, audit, attempt, loaded, stage };
              stretchRescueNationwideGraph = loaded?.graph || stretchRescueNationwideGraph;
            }
            if (!audit.triggered) return { accept:true, suspicious:false, audit };
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
              initialStraightM: Number(perf.routeStretchRescue?.initialStraightM ?? audit.straightM),
              initialRouteM: Number(perf.routeStretchRescue?.initialRouteM ?? audit.routeM),
              initialRatio: Number(perf.routeStretchRescue?.initialRatio ?? audit.ratio),
              initialExcessM: Number(perf.routeStretchRescue?.initialExcessM ?? audit.excessM),
              nationwideStageExpansion:true,
              lastStage:attempt.stage,
              lastBackend:attempt.backend || attempt.requestedBackend || null
            });
            return { accept:false, suspicious:true, audit };
          };
          ...
          for (const stage of nationwideGraphLoadStages()) {
            ...
            try {
              ...
              graphCandidates = await routeLoadedNationwideStage(loaded, attempt, stage, topologyProbeFastestOnly);
              ...
              if (verdict.suspicious) {
                if (shouldJumpToFocusedLiveRescue(verdict.audit)) {
                  earlyLiveStretchRescue = true;
                  perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
                    earlyLiveProbe:true,
                    earlyLiveProbeReason:'extreme-local-stretch',
            ...
            if (acceptedNationwide) break;

            // dev36.1: HGR1 is also a same-window topology cross-check when HGR2
            // returns an implausibly stretched path. This separates HGR2 conversion
            // defects from source-data gaps before widening the corridor.
            const shouldCrossCheckHgr1 = primaryLoaded?.backend === 'nationwide-hgr2' &&
              (!graphCandidates.length || (primarySuspicious && rescueOpts.hgr1SameWindowCrossCheck !== false && !hgr1StretchCrossChecked));
            if (primarySuspicious && shouldCrossCheckHgr1) hgr1StretchCrossChecked = true;
            if (shouldCrossCheckHgr1) {
              try {
                ...
                const attempt = {
                  ...
                  stretchCrossCheck: primarySuspicious,
                  loadedTileCount: Number(loaded?.loadedTileCount || 0),
                  nodeCount: Number(loaded?.nodeCount || 0), edgeCount: Number(loaded?.edgeCount || 0),
                  performance: loaded?.performance || null, routed: false
                };
          ...
          // If every loaded nationwide window still looks stretched, keep the
          // shortest proven graph route as a fallback, but mark it for independent
          // live OSM + direct-corridor evidence rescue below.
          if (!acceptedNationwide && bestStretchedNationwide) {
            graphCandidates = bestStretchedNationwide.candidates;
            graphResult = bestStretchedNationwide.result;
            selectedNationwideFullPlan = {
              loaded: bestStretchedNationwide.loaded,
              attempt: bestStretchedNationwide.attempt,
              stage: bestStretchedNationwide.stage
            };
            runSelectedNationwideFullSearch = async () => {
              if (!selectedNationwideFullPlan) return false;
              graphCandidates = await routeLoadedNationwideStage(
                selectedNationwideFullPlan.loaded,
                selectedNationwideFullPlan.attempt,
                selectedNationwideFullPlan.stage,
                false
              );
              return graphCandidates.length > 0;
            };
            // On an extreme *local* stretch, do not spend temporal/min-sun CPU
            // on a route we already distrust. Focused live/conditional/mature
        ...
        // dev36.1: stretched-route rescue uses cheap topology-only probes first.
        // The expensive shade search is run only after a topology source is selected.
        const preOverpassStretch = routeStretchAudit(aPoint, bPoint, graphCandidates);
        const stretchOpts = routeStretchRescueOptions();
        const topologyProbeFastestOnly = stretchOpts.topologyProbeFastestOnly !== false;
        const needsOverpassRescue = Boolean(graphCandidates.length && preOverpassStretch.triggered && stretchOpts.overpassCrossCheck !== false);
        const overpassBaseOptions = (extra = {}) => Object.assign({
          departure, speedMps, detourPct,
          temporalShadeTableEnabled: config.graphRouting?.temporalShadeTableEnabled,
          temporalShadeTableConcurrency: config.graphRouting?.temporalShadeTableConcurrency,
          temporalShadeTableMaxBucketsPerEdge: config.graphRouting?.temporalShadeTableMaxBucketsPerEdge,
        ...
        if ((!graphCandidates.length || needsOverpassRescue) && config.nationwideTiles?.fallbackToOverpass !== false) {
          const savedGraphCandidates = graphCandidates.slice();
          const savedGraphResult = graphResult;
          ...
          try {
            ...
            perf.graphMs = Number(perf.graphMs || 0) + (nowMs() - overpassStarted);
            if (needsOverpassRescue) {
              const accepted = overpassCandidates.length > 0 && (!Number.isFinite(savedDistanceM) || overpassDistanceM + 1 < savedDistanceM);
              perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
                triggered:true,
                topologyProbeFastestOnly,
                overpassAttempted:true,
                overpassDistanceM:Number.isFinite(overpassDistanceM) ? overpassDistanceM : null,
                overpassAccepted:accepted
              });
              if (accepted) {
                graphResult = overpassResult;
                graphCandidates = overpassCandidates;
                selectedOverpassAllowPrivate = false;
              } else {
                graphResult = savedGraphResult;
                graphCandidates = savedGraphCandidates;
              }
            } else {
              graphResult = overpassResult;
              graphCandidates = overpassCandidates;
            ...
            if (needsOverpassRescue && savedGraphCandidates.length) {
              graphResult = savedGraphResult;
              graphCandidates = savedGraphCandidates;
              perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
                triggered:true,
        ...
        // dev36.3 layer A: exact interior intersection noding. dev36.2 only
        // covered dangling endpoint -> edge near-touches. Some source defects are
        // edge↔edge crossings that are geometrically coincident at grade but lack
        // a shared node. Test those only in a detached strict-live-OSM clone.
        postStrictStretch = routeStretchAudit(aPoint, bPoint, graphCandidates);
        if (stretchRescueCandidates.length === 0 && graphCandidates.length && postStrictStretch.triggered &&
            stretchOpts.interiorNodingRescue !== false && typeof window.HaidianPedestrianGraph?.runInteriorNodingRescue === 'function') {
          try {
            setStatus(`dev36.3：endpoint noding 沒有救回短路；檢查 strict live OSM 裡同層 edge↔edge 幾何交叉但沒有 shared node 的 interior noding defect…`, "loading");
            const interiorStarted = nowMs();
            const repaired = await window.HaidianPedestrianGraph.runInteriorNodingRescue(aPoint, bPoint, {
              departure, speedMps, detourPct,
              corridorM: stretchOpts.interiorNodingCorridorM,
              minImprovementM: stretchOpts.interiorNodingMinImprovementM,
              maxCandidates: stretchOpts.interiorNodingMaxCandidates,
              sharedShadeCache: sharedGraphShadeCache,
              shadeConcurrency: config.graphRouting?.shadeConcurrency,
              shadeEdgeBatchConcurrency: config.graphRouting?.shadeEdgeBatchConcurrency,
              shadeTimeBucketSec: config.graphRouting?.shadeTimeBucketSec,
            ...
            perf.graphMs = Number(perf.graphMs || 0) + (nowMs() - interiorStarted);
            const repairCandidates = Array.isArray(repaired?.candidates) ? repaired.candidates : [];
            const repairedDistanceM = repairCandidates.length ? Math.min(...repairCandidates.map((c)=>Number(c.distanceM||Infinity))) : Number(repaired?.best?.distanceM);
            const accepted = Boolean(repaired?.accepted && repairCandidates.length && Number.isFinite(repairedDistanceM));
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
              interiorNodingAttempted:true,
              interiorNodingAccepted:accepted,
              interiorNodingCandidateCount:Number(repaired?.candidateCount || 0),
              interiorNodingTestedCount:Number(repaired?.testedCount || 0),
              interiorNodingGapM:Number(repaired?.best?.connector?.distanceM ?? 0),
              interiorNodingDistanceM:Number.isFinite(repairedDistanceM) ? repairedDistanceM : null,
              interiorNodingImprovementM:Number(repaired?.best?.improvementM ?? 0),
              interiorNodingClassification:repaired?.best?.spec?.classification || null,
              interiorNodingProductionGraphMutated:false
            });
            if (accepted) {
              for (const c of repairCandidates) stretchRescueCandidates.push(c);
              setStatus(`dev36.3：interior noding rescue 成功；${Math.round(Number(repaired.baselineDistanceM||0))}m → ${Math.round(repairedDistanceM)}m，production graph 未修改。`, "ok");
            }
          } catch (interiorError) {
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true, interiorNodingAttempted:true, interiorNodingAccepted:false,
              interiorNodingError:interiorError?.message || String(interiorError), interiorNodingProductionGraphMutated:false
            });
            console.warn("[Haidian dev36.3 graph] interior noding rescue unavailable; continuing to cross-source corridor rescue.", interiorError);
          }
        }

        // dev36.3 layer B: independent-source corridor union. HGR2 is compiled
        // from Overture and strict live OSM comes from Overpass. If each source is
        // missing a different connector/segment, neither can route the short path
        // alone. Build a detached union and test only tiny endpoint→edge stitches
        // between the two sources (single stitch or bounded two-stitch pair).
        postStrictStretch = routeStretchAudit(aPoint, bPoint, graphCandidates);
        if (stretchRescueCandidates.length === 0 && graphCandidates.length && postStrictStretch.triggered &&
            stretchOpts.crossSourceCorridorRescue !== false && stretchRescueNationwideGraph?.edges?.size &&
            typeof window.HaidianPedestrianGraph?.runCrossSourceCorridorRescue === 'function') {
          try {
            setStatus(`dev36.3：OSM 本身沒有可修的 noding；建立 detached OSM × Overture corridor union，檢查兩份獨立路網是否能互補救回短路徑…`, "loading");
            const xsrcStarted = nowMs();
            const rescued = await window.HaidianPedestrianGraph.runCrossSourceCorridorRescue(aPoint, bPoint, stretchRescueNationwideGraph, {
              departure, speedMps, detourPct,
              snapMaxM: config.graphRouting?.snapMaxM,
              touchMaxM: stretchOpts.crossSourceTouchMaxM,
              reviewMaxM: stretchOpts.crossSourceReviewMaxM,
              corridorM: stretchOpts.crossSourceCorridorM,
              minImprovementM: stretchOpts.crossSourceMinImprovementM,
              maxCandidates: stretchOpts.crossSourceMaxCandidates,
              pairPool: stretchOpts.crossSourcePairPool,
              maxTests: stretchOpts.crossSourceMaxTests,
              sharedShadeCache: sharedGraphShadeCache,
              shadeConcurrency: config.graphRouting?.shadeConcurrency,
              shadeEdgeBatchConcurrency: config.graphRouting?.shadeEdgeBatchConcurrency,
              shadeTimeBucketSec: config.graphRouting?.shadeTimeBucketSec,
            ...
            perf.graphMs = Number(perf.graphMs || 0) + (nowMs() - xsrcStarted);
            const rescueCandidates = Array.isArray(rescued?.candidates) ? rescued.candidates : [];
            const rescuedDistanceM = rescueCandidates.length ? Math.min(...rescueCandidates.map((c)=>Number(c.distanceM||Infinity))) : Number(rescued?.best?.distanceM);
            const accepted = Boolean(rescued?.accepted && rescueCandidates.length && Number.isFinite(rescuedDistanceM));
            const connectorGaps = (rescued?.best?.specs || []).map((x)=>Number(x?.gapM||0));
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
              crossSourceAttempted:true,
              crossSourceAccepted:accepted,
              crossSourceCandidateCount:Number(rescued?.candidateCount || 0),
              crossSourceAutoEligibleCount:Number(rescued?.autoEligibleCount || 0),
              crossSourceReviewOnlyCount:Number(rescued?.reviewOnlyCount || 0),
              crossSourceTestedCount:Number(rescued?.testedCount || 0),
              crossSourceConnectorCount:Number(rescued?.best?.connectors?.length || 0),
              crossSourceGapMaxM:connectorGaps.length ? Math.max(...connectorGaps) : 0,
              crossSourceDistanceM:Number.isFinite(rescuedDistanceM) ? rescuedDistanceM : null,
              crossSourceImprovementM:Number(rescued?.best?.improvementM ?? 0),
              crossSourceProductionGraphMutated:false
            });
            if (accepted) {
              for (const c of rescueCandidates) stretchRescueCandidates.push(c);
              setStatus(`dev36.3：OSM × Overture corridor union 找到較短 source-backed 路徑；${Math.round(Number(rescued.baselineDistanceM||0))}m → ${Math.round(rescuedDistanceM)}m，使用 ${Number(rescued?.best?.connectors?.length||0)} 個 tiny cross-source stitch，production graph 未修改。`, "ok");
            }
          } catch (xsrcError) {
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true, crossSourceAttempted:true, crossSourceAccepted:false,
              crossSourceError:xsrcError?.message || String(xsrcError), crossSourceProductionGraphMutated:false
            });
            console.warn("[Haidian dev36.3 graph] cross-source corridor rescue unavailable; continuing with conditional/mature fallback.", xsrcError);
          }
        }

        // dev37.1 Pedestrian Realm Graph rescue.  dev36.2/36.3 can prove that
        // no small line-graph connector explains an extreme detour.  At that
        // point inspect public pedestrian *areas* (park / pedestrian plaza /
        // square) instead of widening arbitrary connector tolerances.  The realm
        // graph is obstacle-aware, exists only in a detached clone, and every
        // synthetic segment remains explicitly labelled for on-site confirmation.
        postStrictStretch = routeStretchAudit(aPoint, bPoint, graphCandidates);
        if (stretchRescueCandidates.length === 0 && graphCandidates.length && postStrictStretch.triggered &&
            stretchOpts.pedestrianRealmRescue !== false && typeof window.HaidianPedestrianGraph?.runPedestrianRealmRescue === 'function') {
          const baselineDistanceM = graphFastestDistanceM(graphCandidates);
          try {
            setStatus(`dev37.1：line graph 的 noding／跨來源 stitching 都沒有救回短路；改建立 detached Pedestrian Realm Graph，檢查公園／廣場的面狀行人空間與水體／建物／圍籬障礙…`, 'loading');
            const realmStarted = nowMs();
            const preparedRealm=await realmSourcePromise;
            const realm = await window.HaidianPedestrianGraph.runPedestrianRealmRescue(aPoint, bPoint, overpassBaseOptions({
              fastestOnly:false,
              realmSource:preparedRealm.reason==='outside-opportunity-scope'||preparedRealm.reason==='provider-disabled'?undefined:preparedRealm,
              baselineDistanceM,
              marginM: stretchOpts.pedestrianRealmMarginM,
              corridorM: stretchOpts.pedestrianRealmCorridorM,
              minImprovementM: stretchOpts.pedestrianRealmMinImprovementM,
              portalSnapMaxM: stretchOpts.pedestrianRealmPortalSnapMaxM,
              openBoundaryAttachMaxM: stretchOpts.pedestrianRealmOpenBoundaryAttachMaxM,
              openBoundaryBarrierClearanceM: stretchOpts.pedestrianRealmOpenBoundaryBarrierClearanceM,
              boundarySampleM: stretchOpts.pedestrianRealmBoundarySampleM,
              visibilityBoundarySampleM: stretchOpts.pedestrianRealmVisibilityBoundarySampleM,
              maxExplicitPortalsPerArea: stretchOpts.pedestrianRealmMaxExplicitPortalsPerArea,
              maxOpenBoundaryPortals: stretchOpts.pedestrianRealmMaxOpenBoundaryPortals,
              maxBaseNodesPerArea: stretchOpts.pedestrianRealmMaxBaseNodesPerArea,
              maxBoundaryVisibilityNodes: stretchOpts.pedestrianRealmMaxBoundaryVisibilityNodes,
              corridorVisibilitySampleM: stretchOpts.pedestrianRealmCorridorVisibilitySampleM,
              maxCorridorVisibilityNodes: stretchOpts.pedestrianRealmMaxCorridorVisibilityNodes,
              obstacleBoundarySampleM: stretchOpts.pedestrianRealmObstacleBoundarySampleM,
              maxObstacleVisibilityNodes: stretchOpts.pedestrianRealmMaxObstacleVisibilityNodes,
              maxVisibilityM: stretchOpts.pedestrianRealmMaxVisibilityM,
              visibilityNeighborLimit: stretchOpts.pedestrianRealmVisibilityNeighborLimit,
              maxVisibilityEdgesPerArea: stretchOpts.pedestrianRealmMaxVisibilityEdgesPerArea,
              visibilitySampleM: stretchOpts.pedestrianRealmVisibilitySampleM,
              maxSyntheticDistanceM: stretchOpts.pedestrianRealmMaxSyntheticDistanceM,
              maxRouteToStraightRatio: stretchOpts.pedestrianRealmMaxRouteToStraightRatio
            }));
            ...
            const realmMeta = realmCandidates[0]?.graphMeta || {};
            const accepted = Boolean(realm?.accepted && realmCandidates.length && Number.isFinite(realmDistanceM));
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
              pedestrianRealmAttempted:true,
              pedestrianRealmAccepted:accepted,
              pedestrianRealmDistanceM:Number.isFinite(realmDistanceM) ? realmDistanceM : null,
              pedestrianRealmImprovementM:Number(realm?.best?.improvementM ?? realm?.improvementM ?? 0),
              pedestrianRealmAreaCount:Number(realm?.areaCount || 0),
              pedestrianRealmAutoAreaCount:Number(realm?.autoAreaCount || 0),
              pedestrianRealmReviewAreaCount:Number(realm?.reviewAreaCount || 0),
              pedestrianRealmObstacleCount:Number(realm?.obstacleCount || 0),
              pedestrianRealmBarrierCount:Number(realm?.barrierCount || 0),
              pedestrianRealmPortalCount:Number(realm?.portalCount || 0),
              pedestrianRealmExplicitPortalCount:Number(realm?.explicitPortalCount || 0),
              pedestrianRealmOpenBoundaryPortalCount:Number(realm?.openBoundaryPortalCount || 0),
              pedestrianRealmVisibilityEdgeCount:Number(realm?.visibilityEdgeCount || 0),
              pedestrianRealmSyntheticDistanceM:Number(realmMeta.realmSyntheticDistanceM ?? realm?.best?.stats?.syntheticM ?? 0),
              pedestrianRealmSyntheticRatio:Number(realmMeta.realmSyntheticRatio ?? realm?.best?.stats?.syntheticRatio ?? 0),
              pedestrianRealmAreaKinds:(realmMeta.realmAreaKinds || realm?.best?.stats?.kinds || []).slice?.(0,8) || [],
              pedestrianRealmReason:realm?.reason||null,
              pedestrianRealmSource:realm?.realmEndpoint||null,
              pedestrianRealmSourceErrors:(realm?.sourceErrors||[]).slice(0,3),
              pedestrianRealmFeatureCounts:realm?.sourceDiagnostics||null,
              pedestrianRealmRelationCompletion:realm?.relationCompletion||null,
              pedestrianRealmContainmentAttempted:realm?.containmentAttempted===true,
              pedestrianRealmEndpointCoverage:realm?.endpointCoverage||null,
              pedestrianRealmAreaCoverage:(realm?.areaCoverage||[]).slice(0,8),
              pedestrianRealmSourceLinePathDistanceM:Number(realm?.sourceLinePathDistanceM??NaN),
              pedestrianRealmProductionGraphMutated:false,
              pedestrianRealmShadeSearch:realm?.shadeSearch||null,
              pedestrianRealmRequiresOnSitePathConfirmation:true
            });
            if (accepted) {
              for (const c of realmCandidates) stretchRescueCandidates.push(c);
              setStatus(`dev37.1：Pedestrian Realm Graph 找到 ${Math.round(realmDistanceM)}m 的公園／廣場行人空間候選；水體、建物與圍籬作為障礙，production graph 未修改。未映射成明確步道的 realm 區段仍需依現場步道確認。`, 'ok');
            } else {
              setStatus(`dev37.1：已檢查 ${Math.round(Number(realm?.autoAreaCount||0))} 個可自動測試的 public realm；目前仍沒有安全且明顯較短的面狀行人候選，繼續條件式 access / 獨立引擎交叉檢查。`, 'loading');
            }
          } catch (realmError) {
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true, pedestrianRealmAttempted:true, pedestrianRealmAccepted:false,
              pedestrianRealmError:realmError?.message || String(realmError),pedestrianRealmReason:'realm-source-unavailable', pedestrianRealmProductionGraphMutated:false
            });
            console.warn('[Haidian dev37.1 graph] pedestrian realm rescue unavailable; continuing with conditional/mature fallback.', realmError);
          }
        }
        ...
        if (stretchRescueCandidates.length === 0 && graphCandidates.length && postStrictStretch.triggered && stretchOpts.conditionalPrivateAccessProbe !== false) {
          ...
          try {
            ...
            const privateProbe = await window.HaidianPedestrianGraph.findRoutes(
              ...
              overpassBaseOptions({
                fastestOnly:true,
                allowPrivateFootAccess:true,
                conditionalPrivateTerminalBufferM: stretchOpts.conditionalPrivateTerminalBufferM
              })
            );
            perf.graphMs = Number(perf.graphMs || 0) + (nowMs() - privateStarted);
            const privateCandidates = Array.isArray(privateProbe?.candidates) ? privateProbe.candidates : [];
            const privateFastest = graphFastestCandidate(privateCandidates);
            const privateDistanceM = graphFastestDistanceM(privateCandidates);
            const accessMeta = privateFastest?.graphMeta || {};
            const improvementM = Number.isFinite(beforePrivateDistanceM) && Number.isFinite(privateDistanceM)
              ? beforePrivateDistanceM - privateDistanceM
              : -Infinity;
            const minImprovementM = Math.max(20, Number(stretchOpts.conditionalPrivateAccessMinImprovementM) || 120);
            const accessIsTerminalSafe = accessMeta.usesConditionalPrivateAccess === true && accessMeta.privateAccessTerminalOnly === true;
            const accepted = privateCandidates.length > 0 && accessIsTerminalSafe && improvementM >= minImprovementM;
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
              conditionalAccessAttempted:true,
              conditionalAccessDistanceM:Number.isFinite(privateDistanceM) ? privateDistanceM : null,
              conditionalAccessImprovementM:Number.isFinite(improvementM) ? improvementM : null,
              conditionalAccessPrivateDistanceM:Number(accessMeta.privateAccessDistanceM || 0),
              conditionalAccessPrivateInteriorM:Number(accessMeta.privateAccessInteriorDistanceM || 0),
              conditionalAccessTerminalOnly:Boolean(accessMeta.privateAccessTerminalOnly),
              conditionalAccessAccepted:accepted
            });
            if (accepted) {
              graphResult = privateProbe;
              graphCandidates = privateCandidates;
              selectedOverpassAllowPrivate = true;
              postStrictStretch = routeStretchAudit(aPoint, bPoint, graphCandidates);
            } else {
              graphResult = beforePrivateResult;
              graphCandidates = beforePrivateCandidates;
            }
          } catch (privateError) {
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
        ...
        // Independent mature-engine cross-check. This never mutates our graph and
        // is kept as a separately-labelled comparison candidate. It is especially
        ...
        if (stretchRescueCandidates.length === 0 && graphCandidates.length && postPrivateStretch.triggered && stretchOpts.matureValhallaProbe !== false && typeof window.HaidianPedestrianGraph?.fetchValhallaPedestrianRoute === "function") {
          ...
          try {
            ...
            perf.graphMs = Number(perf.graphMs || 0) + (nowMs() - matureStarted);
            const matureDistanceM = Number(mature?.candidate?.distanceM);
            const improvementM = Number.isFinite(currentDistanceM) && Number.isFinite(matureDistanceM)
              ? currentDistanceM - matureDistanceM
              : -Infinity;
            const minImprovementM = Math.max(20, Number(stretchOpts.matureRescueMinImprovementM) || 120);
            const accepted = Boolean(mature?.available && mature?.candidate && Number.isFinite(matureDistanceM) && improvementM >= minImprovementM);
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              triggered:true,
              valhallaAttempted:true,
              valhallaDistanceM:Number.isFinite(matureDistanceM) ? matureDistanceM : null,
              valhallaImprovementM:Number.isFinite(improvementM) ? improvementM : null,
              valhallaAccepted:accepted,
              valhallaError:mature?.available === false ? (mature?.reason || null) : null
            });
            if (accepted) {
              stretchRescueCandidates.push(Object.assign({}, mature.candidate, {
                rescueMeta:{ source:'valhalla', productionGraphMutated:false, reason:'route-stretch-independent-crosscheck' }
              }));
            }
          } catch (matureError) {
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
        ...
        // A strict/conditional Overpass rescue above may intentionally have been
        // topology-only. Once the winning live source is known, run min-sun once.
        if (selectedOverpassAllowPrivate !== null && (graphResult?.topologyOnly === true || graphResult?.diagnostics?.topologyOnly === true)) {
          const probeCandidates = graphCandidates.slice();
          ...
          try {
            ...
            perf.graphMs = Number(perf.graphMs || 0) + (nowMs() - fullStarted);
            const fullCandidates = Array.isArray(fullOverpass?.candidates) ? fullOverpass.candidates : [];
            if (fullCandidates.length) {
              graphResult = fullOverpass;
              graphCandidates = fullCandidates;
            }
          } catch (fullError) {
            graphResult = probeResult;
            graphCandidates = probeCandidates;
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
        ...
        // If the focused rescue stack found nothing shorter, preserve the old
        // functionality by paying for one full static min-sun search now. This
        // is intentionally last, so a 7× local stretch no longer triggers both
        // 25-tile expansion and expensive shade work before live evidence.
        if (earlyLiveStretchRescue && selectedOverpassAllowPrivate === null && stretchRescueCandidates.length === 0 &&
          ...
          try {
            ...
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              staticFullSearchDeferred:true,
              staticFullSearchFallbackExecuted:true,
              staticFullSearchFallbackAvailable:Boolean(ok)
            });
          } catch (fallbackError) {
            perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
              staticFullSearchDeferred:true,
      ...
      // dev37.2 ordinary realm opportunity is independent from topology rescue.
      // No second source request or duplicate realm search after a rescue probe.
      if(graphCandidates.length&&!perf.routeStretchRescue?.pedestrianRealmAttempted){
        ...
        if(source.available){
          const started=nowMs();
          try{
            setStatus('檢查附近公園／廣場的內部遮蔭替代路徑…','loading');
            const realm=await window.HaidianPedestrianGraph.runPedestrianRealmRescue(aPoint,bPoint,{
            ...
            if(serial!==analysisSerial)return;
            Object.assign(perf.realmOpportunity,{accepted:false,reason:error?.message||String(error)});
          }finally{perf.realmOpportunity.computeMs=nowMs()-started;}
        }
      }
      ...
      if (graphResult?.available !== false && graphCandidates.length && config.autoCompareVerifiedFusion !== false) {
        ...
        try {
          const finalStretchAudit = routeStretchAudit(aPoint, bPoint, graphCandidates);
          const rescueOpts = routeStretchRescueOptions();
          const rescueDistances = [
            perf.routeStretchRescue?.localNodingAccepted ? Number(perf.routeStretchRescue?.localNodingDistanceM) : NaN,
            perf.routeStretchRescue?.interiorNodingAccepted ? Number(perf.routeStretchRescue?.interiorNodingDistanceM) : NaN,
            perf.routeStretchRescue?.crossSourceAccepted ? Number(perf.routeStretchRescue?.crossSourceDistanceM) : NaN,
            perf.routeStretchRescue?.pedestrianRealmAccepted ? Number(perf.routeStretchRescue?.pedestrianRealmDistanceM) : NaN,
            perf.routeStretchRescue?.conditionalAccessAccepted ? Number(perf.routeStretchRescue?.conditionalAccessDistanceM) : NaN,
            perf.routeStretchRescue?.valhallaAccepted ? Number(perf.routeStretchRescue?.valhallaDistanceM) : NaN
          ].filter(Number.isFinite);
          const acceptedRescueDistanceM = rescueDistances.length ? Math.min(...rescueDistances) : NaN;
          const finalComparableRouteM = Number.isFinite(acceptedRescueDistanceM) ? acceptedRescueDistanceM : Number(finalStretchAudit.routeM);
          const finalComparableRatio = Number.isFinite(finalComparableRouteM) && Number(finalStretchAudit.straightM) > 0
            ? finalComparableRouteM / Number(finalStretchAudit.straightM)
            : Number(finalStretchAudit.ratio);
          const finalComparableExcessM = Number.isFinite(finalComparableRouteM) && Number.isFinite(Number(finalStretchAudit.straightM))
            ? Math.max(0, finalComparableRouteM - Number(finalStretchAudit.straightM))
            : Number(finalStretchAudit.excessM);
          const directEvidenceProbe = Boolean(finalStretchAudit.triggered && rescueOpts.directEvidenceProbe !== false && !Number.isFinite(acceptedRescueDistanceM));
          const graphRoutePolylines = graphCandidates.filter((c) => Array.isArray(c?.points) && c.points.length >= 2).map((c) => c.points);
          // dev36.1 breaks the old circular blind spot: when the graph itself is
          // detouring, discovery must not inspect only that detour.  Probe the
          // short A→B corridor for independent official source-following geometry.
          const discoveryPolylines = directEvidenceProbe ? [[aPoint, bPoint]] : graphRoutePolylines;
          const primaryEvidenceSeed = directEvidenceProbe
            ? [aPoint, bPoint]
            : ((graphCandidates.find((c) => c.kind === "graph-shade") || graphCandidates[0])?.points || nationwideSeed);
          const evidenceMarginM = directEvidenceProbe
            ? Number(rescueOpts.directEvidenceMarginM || 260)
            : (config.officialEvidenceDiscovery?.routeCorridorM ?? config.nationwideTiles?.routeBufferM);
          perf.routeStretchRescue = Object.assign({}, perf.routeStretchRescue || {}, {
            triggered:Boolean(perf.routeStretchRescue?.triggered || finalStretchAudit.triggered),
            finalStraightM:finalStretchAudit.straightM,
            finalRouteM:finalComparableRouteM,
            finalRatio:finalComparableRatio,
            finalExcessM:finalComparableExcessM,
            acceptedRescueDistanceM:Number.isFinite(acceptedRescueDistanceM) ? acceptedRescueDistanceM : null,
            directEvidenceProbe
          });
          setStatus(directEvidenceProbe
            ? `dev37.1 Route Stretch Rescue：線網與 Pedestrian Realm 仍未救回短路徑；改掃 A→B 直接 corridor 的官方步行 evidence…`
          ...
          if (serial !== analysisSerial) return;
          setStatus(directEvidenceProbe
            ? "dev37.1：在 detached local graph 嘗試 direct-corridor verified official-fusion rescue…"
          ...
          experimentalFusion = await buildAutomaticExperimentalFusionCandidate({
            ...
            speedMps,
            detourPct,
            routePolylines: discoveryPolylines,
      ...
      bundle.candidateAudit = candidateAudit;
  ...
  window.HaidianRouteExposure = {
    ...
    get lastCandidateCorrectnessAudit() { return lastCandidateCorrectnessAudit; },
    ...
    _internals: {
      ...
      graphFastestDistanceM,
      routeStretchAudit,
      activeEvidenceRouteLines,
      routeNearKnownEvidenceIndex,
