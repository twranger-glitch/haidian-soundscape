 ...
 * v8.9.0-dev1 route integration: preserves UI and adds programmatic point APIs.
...
(function () {
  ...
  function normalizedPoint(lat, lng) {
    ...
    if (!Number.isFinite(point.lat) || !Number.isFinite(point.lng)) {
      throw new Error("Invalid heat-risk coordinate");
    }
    return point;
  }

  async function fetchProgrammatic(path, lat, lng, options = {}) {
    if (!API_BASE.startsWith("https://") || API_BASE.includes("REPLACE-WITH")) {
      throw new Error("Heat-risk service URL is not configured");
    }
    const point = normalizedPoint(lat, lng);
    const externalSignal = options.signal || null;
    const localController = externalSignal ? null : new AbortController();
    const signal = externalSignal || localController.signal;
    const timeoutMs = Math.max(500, Number(options.timeoutMs) || TIMEOUT_MS);
    const timer = localController ? window.setTimeout(() => localController.abort(), timeoutMs) : null;
    try {
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok || !payload?.ok) {
        const message = path === "/temperature-point"
          ? cwaGridErrorMessage(payload, response.status)
          : friendlyError(payload, response.status);
  ...
  async function assessPoint(lat, lng, options = {}) {
    return fetchProgrammatic("/risk", lat, lng, options);
  }

  async function temperaturePoint(lat, lng, options = {}) {
    return fetchProgrammatic("/temperature-point", lat, lng, options);
  }
  ...
  function addButton() {
     ...
     * 耳機 → 熱風險小太陽 → 圖層 → 圖資面板。
     *
     * 不使用 menu.previousElementSibling，
     * 避免收合耳朵介入後抓錯按鈕。
     */
  ...
  window.HaidianHeatRisk = Object.assign({}, window.HaidianHeatRisk || {}, {
    version: "v8.9.0-dev1",
    apiBase: API_BASE,
    assessPoint,
    temperaturePoint
