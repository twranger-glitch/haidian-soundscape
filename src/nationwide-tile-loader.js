/*
 * Haidian Soundscape — Taiwan Nationwide Tile Loader v9.0.0-dev34.5 Connectivity-aware Endpoint Snap Runtime
 *
  ...
  async function loadGraphForBBox(bbox, options = {}) {
    ...
    if ((options.preferHgr2 ?? config.preferHgr2) !== false && hasGraph2(manifest)) {
      try {
        ...
        console.warn('[Haidian dev34.5 nationwide graph] HGR2 load failed; trying HGR1 for the same bbox.', error);
      }
    }
    const started = nowMs();
    let t = nowMs();
